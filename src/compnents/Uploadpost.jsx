...
function Uploadpost() {
  ...
  function submitHandler(e) {
    ...
    console.log(data);
    dispatch(uploadPost(data, token));
    ...
    <div className=' container-fluid  border border-1 sm-4 bg-light '>
      <div className=' row d-flex align-items-center ' >
        <div className='offset-1 col-sm-2'>
          <img src={himanshi} class="rounded-circle w-25" alt="..." />

        </div>
        <div className='col-sm-7 pt-2'>
          <form className='d-flex ' onSubmit={submitHandler} >
            <input type='text' onChange={changeHandler1} name='content' value={content} placeholder='start a post.....' className='rounded-pill  sm border border-1  post-text w-100 rounded-3 bg-light  ' />
            <input id="icon-button-file" type='file'  onChange={changeHandler2} className=' form-control-files  ' accept='image/*' multiple />
          
            <input className='border-0 px-4 py-2' type='submit' value='Post' />
          </form>
        </div>
